...
int main(int argc, char* argv[]) {
	...
	while (true) {
		...
		if (CreateIoCompletionPort((HANDLE)perHandleData[nClients]->socket, completionPort, (DWORD)perHandleData[nClients], 0) == NULL) {
			cout << "CreateIoCompletionPort() failed with error " << GetLastError() << endl;
			return 1;
		}
...
unsigned __stdcall serverWorkerThread(LPVOID completionPortID) {
	...
	while (TRUE) {
		...
		status = GetQueuedCompletionStatus(completionPort, &transferredBytes, (LPDWORD)&pHD, (LPOVERLAPPED*)&pID, INFINITE);
